...
const Navbar = () => {
  ...
  return (
    ...
    >
      ...
      <p className='text-white text-[18px] font-bold cursor-pointer flex'>Ramzi &nbsp; <span className='sm:block hidden'> | Full Stack Developer</span></p>
      </Link>
    ...
    <a href={`#${link.id}`}>{link.title}</a>
      ...
      <div className={`${!toggle ? 'hidden' : 'flex'} p-6 black-gradient absolute top-20 right-20 mx4 my-2 min-w-[140px] z-10 rounded-xl`}>
      <ul className='list-none flex sm:flex justify-end items-start flex-col gap-4'>
    {navLinks.map((link)=>(
    <li key={link.id} className={`${active===link.title
    ? "text-white" : "text-secondary"
    } font-poopins font-medium text-[16px] cursor-pointer`} onClick={()=>{setActive(link.title);setToggle(!toggle)}}>
    <a href={`#${link.id}`}>{link.title}</a>
  </li>
    ))}
